...
//Zero angle
int zero_angle=280; //anti-clock-wise is increasing
int norm_IMU_temp=0;
...
int rud_range_min = 50;  // available rudder range minimun, vary from boat to boat
int rud_range_max = 170;  // available rudder range maximun, vary from boat to boat
String inString="";
char c;
...
PID myPID(&Input, &Output, &Setpoint,2.5,0,0.01, DIRECT);

void setup() 
{
  JY901.StartIIC(0x50);
  myservo1.attach(10);//the servo control used PWM
  myservo2.attach(9);//the servo control used PWM
  Serial.begin(115200);
  while (!Serial) {
...
{  
  ...
  Serial.print(360-((int)((float)JY901.stcAngle.Angle[2]/32768*180)+180));
  Serial.print(" h"); Serial.print(int(Setpoint));

  int IMU_temp=360-((int)((float)JY901.stcAngle.Angle[2]/32768*180)+180);
